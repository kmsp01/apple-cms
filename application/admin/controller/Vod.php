...
class Vod extends Base
{
    ...
    public function data()
    {
        ...
        if(!empty($param['points'])){
            $where['vod_points_play|vod_points|vod_points_down'] = ['gt', 0];
        }
        if(!empty($param['pic'])){
            if($param['pic'] == '1'){
                $where['vod_pic'] = ['eq',''];
    ...
    public function batch()
    {
        ...
        if (!empty($param)) {
            ...
            foreach($res['list'] as  $k=>$v){
				...
				if(!empty($param['ck_points']) && $param['val_points_all']!=''  ){
                    $update['vod_points'] = $param['val_points_all'];
                    $des .= '&nbsp;'.lang('points_all').'：'.$param['val_points_all'].'；';
                }
