...
class Base extends All
{
    ...
    protected function check_user_popedom($type_id,$popedom,$param=[],$flag='',$info=[],$trysee=0)
    {
        ...
        elseif($popedom==2 && in_array($pre,['art','actor','website'])){
            ...
            elseif($group['group_id']<4 && $points>0  ){
                $mid = mac_get_mid($pre);
                $where=[];
                $where['ulog_mid'] = $mid;
                $where['ulog_type'] = 1;
        ...
        elseif($popedom==3){
            ...
            elseif($group['group_id']<4 && empty($group['group_popedom'][$type_id][3]) && !empty($group['group_popedom'][$type_id][5]) && $trysee>0){
            ...
            elseif($popedom==5){
                if(empty($group['group_popedom'][$type_id][3]) && !empty($group['group_popedom'][$type_id][5])){
                    ...
                    elseif( $group['group_id'] <4 && $points > intval($user['user_points']) ){
